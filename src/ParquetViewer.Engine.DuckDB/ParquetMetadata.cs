...
namespace ParquetViewer.Engine.DuckDB
{
    public class ParquetMetadata : IParquetMetadata
    {
        ...
        public static async Task<ParquetMetadata> FromDuckDBAsync(DuckDBHandle db)
        {
            ...
            await foreach (var row in db.Connection.QueryAsync($"SELECT * FROM parquet_metadata('{db.ParquetFilePath}');"))
            {
            ...
            var fileMetadata = await db.Connection.QuerySingleAsync($"SELECT * FROM parquet_file_metadata('{db.ParquetFilePath}');");
            var createdBy = fileMetadata.GetString(1);
            ...
            var metadata = new ParquetMetadata(schemaTree, rowGroups, (int)numRows, (int)parquetVersion, createdBy, (int)numRowGroups);
            return metadata;
        }
    }
